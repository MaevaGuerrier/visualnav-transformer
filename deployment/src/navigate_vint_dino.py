...
model_params = {"normalize": True, "context_size": 5, "image_size": [192, 192]}
...
def main(args: argparse.Namespace):
    ...
    vint_onnx = load_model_onnx(args.model)
    print("loaded model")
    # load topomap
    ...
    while not rospy.is_shutdown():
        ...
        if len(context_queue) > model_params["context_size"]:
                ...
                print(batch_obs_imgs_np.shape)
                print(batch_goal_data_np.shape)
        ...
        # RECOVERY MODE
        if model_params["normalize"] and not args.metric:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metric",
        action="store_true",
        help="If set, will treat the model output as metric predictions"
    )
    parser.add_argument(
        "--radius",
        "-r",
