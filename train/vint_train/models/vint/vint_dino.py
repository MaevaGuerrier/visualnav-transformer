...
class ViNTWithDINOTokens(BaseModel):
    def __init__(
        ...
        # token embedding for <pred>
        self.token_embedding = nn.Embedding(
            num_embeddings=1,
            embedding_dim=self.encoding_size,
        )
        # positional embedding for temporal steps
        ...
        if positional_encoding_type == "rope":
            ...
            # dim for each dimension (h and w)
        ...
        self.decoder = LastTokenMultiLayerDecoder(
            ...
            seq_len=(self.context_size+1)*(self.image_size[0]//self.patch_size)*(self.image_size[1]//self.patch_size)+1,
    ...
    def forward(
        ...
        # obs_image has context_size+1 images somehow (original author bug?)
        obs_img_split = torch.split(obs_img, 3, dim=1)[1:]

        # image size is [batch_size*self.context_size + batch_size, 3, H, W]
        img = torch.concat((*obs_img_split, goal_img), dim=0)

        # get the image encoding
        vision_outputs = self.vision_encoder(pixel_values=img)
        # discard cls and reg tokens then reshape
        ...
        ) # (batch_size*self.context_size + batch_size, hidden_size, patch_H, patch_W)
        # (batch_size*self.context_size + batch_size, hidden_size, n_img_tokens)
        vision_encodings = self.vision_projector(vision_outputs)

        # interleave the token embeddings with the image encodings
        obs_tokens = vision_encodings[:batch_size*self.context_size, :, :].reshape(
            batch_size,
            self.context_size,
            self.encoding_size,
            self.image_size[0]//self.patch_size//2 * self.image_size[1]//self.patch_size//2,
        )
        goal_tokens = vision_encodings[batch_size*self.context_size:, :, :].reshape(
            batch_size,
            1,
            self.encoding_size,
            self.image_size[0]//self.patch_size//2 * self.image_size[1]//self.patch_size//2,
        ...
        image_tokens = torch.cat([obs_tokens, goal_tokens], dim=1)  # (batch_size, context_size+1, encoding_size, n_img_tokens)
        ...
        if self.positional_encoding_type == "rope":
            # 2d rope (split embeddings in half for H and W)
            ...
            # cos_h/sin_h: (grid_h, dim)
            # We want (1, 1, dim, grid_h, 1)
            ...
            # cos_w/sin_w: (grid_w, dim)
            # We want (1, 1, dim, 1, grid_w)
        ...
        # Now the tokens look like:
        # [obs_1_tokens, obs_2_tokens, ..., obs_n_tokens, goal_tokens, </pred>]
        tokens = torch.cat([
            image_tokens.reshape(
                batch_size,
                self.encoding_size,
                -1
            ),  # (batch_size, encoding_size, (context_size+1)*n_img_tokens)
            pred_token_emb
        ], dim=-1)
        tokens = torch.transpose(tokens, 1, 2)  # (batch_size, total_tokens, encoding_size)
